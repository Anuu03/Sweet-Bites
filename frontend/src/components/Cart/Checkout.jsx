...
  // This useEffect is causing the issue. Let's make it smarter.
  useEffect(() => {
...
  // ✅ Step 2a: Razorpay success handler (Uses Redux thunks)
...
      } else {
        console.error("Finalize failed after payment.");
      }
    } else {
...
  // ✅ Step 2b: COD Finalization handler (Uses Redux thunks)
  const handleCodPayment = async () => {
    // The createCheckout call should have already happened.
    const payRes = await dispatch(
      payCheckout({
        checkoutId,
...
        navigate("/order-confirmation");
      } else {
        console.error("Finalize failed after COD.");
      }
    } else {
      console.error("COD payment update failed.");
    }
  };
