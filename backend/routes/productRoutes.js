...
// ✅ FIX: Place more specific routes before the generic '/:id' route
...
router.get("/best-seller", async (req, res) => {
    try {
        // First, try to find products with sold > 0
        let bestSeller = await Product.findOne({ sold: { $gt: 0 } })
            .sort({ sold: -1 })
            .limit(1);

        // If no product has been sold yet, return the most recent published product
        if (!bestSeller) {
        ...
        if (bestSeller) {
            ...
            res.status(404).json({ message: "No products found in database" });
        }
    } catch (error) {
